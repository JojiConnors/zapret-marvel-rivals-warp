...
Адаптация сборки `flowseal/zapret-discord-youtube` для запуска Cloudflare WARP
и Marvel Rivals через Steam. Подготовил: **JojiConnors**.

Для запуска используйте файлы:

- `1 - INSTALL Cloudflare WARP.bat` — открывает официальный установщик Cloudflare;
- `2 - START Marvel Rivals via WARP.bat` — выбор профиля zapret, подключение WARP
  через MASQUE и последующее выключение по нажатию Enter;
- `3 - STOP WARP and zapret.bat` — аварийное отключение и очистка служб.

Полная инструкция на русском и английском находится в
[`README - Marvel Rivals WARP.txt`](./README%20-%20Marvel%20Rivals%20WARP.txt).

> Работа конкретного профиля зависит от интернет-провайдера. Cloudflare WARP,
> Steam и Marvel Rivals не входят в архив и устанавливаются отдельно.
