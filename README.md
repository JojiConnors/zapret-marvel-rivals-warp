...
В качестве отдельного эксперимента можно вместо первого файла запустить
`4_START_ALL_TCP_UDP_TEST.bat`. Он повторяет широкий пресет Zapret 2
`ALL TCP & UDP hostfakesplit_multi_syndata`. Не запускайте оба профиля
одновременно. Если вырос пинг или нарушилась работа других программ,
остановите тест через `2_STOP.bat` и вернитесь к основному профилю.
